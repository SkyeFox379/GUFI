...
def create_tree(current_dir, layer_num):
    ...
    # 1 in 20 chance to create a dead end directory before the max depth is reached
    if layer_num >= max_depth or random.randrange(1,20) == 1:
...
create_tree(os.path.join(".", str(args.max_width) + "wideTree"), 0)
